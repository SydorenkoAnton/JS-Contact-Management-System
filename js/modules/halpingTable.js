export function getNames(client) {                                                               // для таблицы создаем ФИО
  const clientName = client.surname + ' ' + client.name + ' ' + client.lastName;
  return clientName;
}

function giveZero(number) {                                                                       // добавляем 0 если однозначное число
  if(number < 10) number = "0" + number;
  return number;
}

export function getMakingDate(client) {                                                           // для таблицы создаем дату
  let startDate = new Date(client);
  let [month, day, year] = [startDate.getMonth(), startDate.getDate(), startDate.getFullYear()];  // берем месяц, день, год с Даты
  day = giveZero(day);                                                                            // проверим на однозначное число
  month = ++month;                                                                                // добавим 1 к месяцу
  month = giveZero(month);                                                                        // проверим на однозначное число
  const fullDate = `${day}` + '.' + `${month}` + '.' + `${year}`                                  // итоговая дата
  return fullDate;
}
// для таблицы - время создания
export function getMakingTime(client) {                                                           // для таблицы - время
  let startDate = new Date(client);
  let [hours, minutes] = [startDate.getHours(), startDate.getMinutes()];                          // берем часы и минуты
  hours = giveZero(hours);                                                                        // проверим на однозначное число
  minutes = giveZero(minutes);
  const fullTime = `${hours}` + ':' + `${minutes}`                                                // итоговое время
  return fullTime;
}

function makeContact(contact) {                                                                   // в зависимости от типа создаем контакт
  const contactLink = document.createElement('a');                                                // создаем ссылку
  contactLink.classList = "table__contakts";                                                      // добавляем класс
  if (contact.type == 'vkontakte') {                                                              // если тип 'vkontakte'
    contactLink.href = `${contact.value}`                                                         // адрес ссылки берем с contact.value
    contactLink.datatooltip = `<span class='table__contakts-descr'>vk: </span><a class='table__contakts-link'>${contact.value}</a>`;  // инфа для tooltip
  ...
  if (contact.type == 'телефон') {                                                                // если тип 'телефон'
    contactLink.href = `tel:${contact.value}`                                                     // номер телефона берем с contact.value
    contactLink.datatooltip = `<a class='table__contakts-tel' style="width: 110px">${contact.value}</a>`;                              // инфа для tooltip
    contactLink.innerHTML = `
      <svg class="table__contakts-svg" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g opacity="0.7">
  ...
  if (contact.type == 'Facebook') {                                                               // если тип 'Facebook'
    contactLink.href = `${contact.value}`                                                         // адрес ссылки берем с contact.value
    contactLink.datatooltip = `<span class='table__contakts-descr'>fb: </span><a class='table__contakts-link'>${contact.value}</a>`;    // инфа для tooltip
    contactLink.innerHTML = `
      <svg class="table__contakts-svg" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  if (contact.type == 'Email') {                                                                  // если тип 'Email'
    contactLink.href = `mailto:${contact.value}`                                                  // адрес ссылки берем с contact.value
    contactLink.datatooltip = `<span class='table__contakts-descr'>email: </span><a class='table__contakts-link'>${contact.value}</a>`;    // инфа для tooltip
    contactLink.innerHTML = `
      <svg class="table__contakts-svg" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  if (contact.type == 'другое') {                                                                  // если тип 'другое'
    contactLink.href = `${contact.value}`                                                          // адрес ссылки берем с contact.value
    contactLink.datatooltip = `<span class='table__contakts-descr'>email: </span><a class='table__contakts-link'>${contact.value}</a>`;    // инфа для tooltip
    contactLink.innerHTML = `
...
export function getContacs(client) {                                                              // для таблицы - создаем блок контактов
  ...
  for (let contact of filtrContacts) {                                                            // для каждого контакта
    ...
    if (contactsCounter >= 4){                                                                    // если больше 4 контактов добавляем контакты в скрытый блок
      ...
      contactLink = makeContact(contact);                                                         // создаем остальные контакты
      let disableContacts = [];
